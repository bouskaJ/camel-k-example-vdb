...
export IMAGE=quay.io/teiid/teiid-operator:0.1.0
...
if $(check_resource deployment ${RESOURCE}) ; then 
  ...
  wget -O operator.yaml $OP_ROOT/operator.yaml
  sed "s|\$IMAGE_LOCATION|${IMAGE}|g" operator.yaml > operatorx.yaml
  oc create -f operatorx.yaml
  rm operator.yaml operatorx.yaml
fi